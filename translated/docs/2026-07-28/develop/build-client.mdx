---
title: "构建 MCP 客户端"
description: "开始构建你自己的、能够与所有 MCP 服务器集成的客户端。"
---

在本教程中，你将学习如何构建一个由 LLM 驱动的聊天机器人客户端，它连接到 MCP 服务器。

在开始之前，先完成我们的[构建 MCP 服务器](/docs/2026-07-28/develop/build-server)教程会有帮助，这样你就能理解客户端和服务器如何通信。
...
你将需要一个来自 [Anthropic Console](https://console.anthropic.com/settings/keys) 的 Anthropic API 密钥。
...
那个 `async with` 就是整个连接生命周期。进入它会启动服务器并与它协商一个协议版本；离开它会断开连接并关闭子进程。没有什么需要手动关闭。
...
如果你正在继续[服务器快速开始中的天气教程](https://github.com/modelcontextprotocol/quickstart-resources/tree/main/weather-server-python)，你的命令可能看起来像这样：`python client.py .../quickstart-resources/weather-server-python/weather.py`
...
4. **工具名称**
   - 工具名称可以根据[这里](/specification/2026-07-28/server/tools#tool-names)指定的格式进行校验
...
更新你的 `package.json` 以设置 `type: "module"` 和一个 build 脚本：
...
首先，让我们在 `index.ts` 中设置我们的导入并创建基本的客户端类：
...
如果你正在继续[服务器快速开始中的天气教程](https://github.com/modelcontextprotocol/quickstart-resources/tree/main/weather-server-typescript)，你的命令可能看起来像这样：`node build/index.js .../quickstart-resources/weather-server-typescript/build/index.js`
...
- 如果相对路径不起作用，使用绝对路径
- 对于 Windows 用户，确保在路径中使用正斜杠（/）或转义的反斜杠（\\）
- 验证服务器文件有正确的扩展名（Node.js 为 .js 或 Python 为 .py）
...
- `Error: Cannot find module`：检查你的 build 文件夹并确保 TypeScript 编译成功
- `Connection refused`：确保服务器正在运行且路径正确
- `Tool execution failed`：验证工具所需的环境变量已设置
...
这是一个基于 Spring AI MCP 自动配置和 boot starter 的快速开始演示。要了解如何手动创建同步和异步 MCP 客户端，请查阅 [Java SDK Client](https://java.sdk.modelcontextprotocol.io/) 文档。

</Note>

本示例演示如何构建一个交互式聊天机器人，它将 Spring AI 的模型上下文协议（MCP）与 [Brave Search MCP Server](https://github.com/modelcontextprotocol/servers-archived/tree/main/src/brave-search) 结合起来。该应用创建一个由 Anthropic 的 Claude AI 模型驱动的对话界面，它可以通过 Brave Search 执行互联网搜索，从而实现与实时 Web 数据的自然语言交互。[你可以在这里找到本教程的完整代码。](https://github.com/spring-projects/spring-ai-examples/tree/main/model-context-protocol/web-search/brave-chatbot)
...
确保你妥善保管你的 `ANTHROPIC_API_KEY` 和 `BRAVE_API_KEY` 密钥！
...
这会激活 `spring-ai-starter-mcp-client`，以基于所提供的服务器配置创建一个或多个 `McpClient`。`spring.ai.mcp.client.toolcallback.enabled=true` 属性启用工具回调机制，该机制自动将所有 MCP 工具注册为 Spring AI 工具。它默认是禁用的。
...
- 通过 MCP 集成 Brave Search 以获得实时 Web 搜索能力
...
该应用将启动一个交互式聊天会话，你可以在其中提问。当聊天机器人需要从互联网查找信息来回答你的查询时，它将使用 Brave Search。
...
- 具有多种传输类型的多个客户端：`STDIO` 和 Streamable HTTP
- 与 Spring AI 的工具执行框架集成
- 自动的客户端初始化和生命周期管理

要通过 Streamable HTTP 连接到远程 MCP 服务器，配置一个连接 URL：

```properties
spring.ai.mcp.client.streamable-http.connections.server1.url=http://localhost:8080
```

对于基于 WebFlux 的应用，你可以改用 WebFlux starter：
...
这提供类似的功能，但使用基于 WebFlux 的 Streamable HTTP 传输实现，推荐用于生产部署。
...
<Accordion title="JsonObject.toJsonValue() 辅助函数">

这个辅助函数使用 Jackson 将一个 kotlinx.serialization 的 `JsonObject` 转换为一个 Anthropic SDK 的 `JsonValue`：
...
如果你正在继续服务器快速开始中的天气教程，你的命令可能看起来像这样：`java -jar build/libs/kotlin-mcp-client-0.1.0-all.jar .../samples/weather-stdio-server/build/libs/weather-stdio-server-0.1.0-all.jar`
...
1. **错误处理**
   - 利用 Kotlin 的类型系统来显式地建模错误
   - 在可能发生异常时将外部工具和 API 调用包裹在 `try-catch` 块中
   - 提供清晰而有意义的错误消息
   - 优雅地处理网络超时和连接问题

2. **安全**
   - 将 API 密钥和密钥安全地存储在 `local.properties`、环境变量或密钥管理器中
   - 校验所有外部响应以避免意外或不安全的数据使用
   - 在使用工具时对权限和信任边界保持谨慎

3. **环境**
   - 通过环境变量设置 `ANTHROPIC_API_KEY`，而不是硬编码
   - 使用带有适当 `.gitignore` 规则的 `.env` 文件用于本地开发
...
- 如果相对路径不起作用，使用绝对路径
- 对于 Windows 用户，确保在路径中使用正斜杠（/）或转义的反斜杠（\\）
- 确保所需的运行时已安装（Java 需要 java，Node.js 需要 npm，Python 需要 uv）
- 验证服务器文件有正确的扩展名（Java 为 .jar，Node.js 为 .js 或 Python 为 .py）
...
- 如果你遇到 JDK 版本错误，确保你安装的 JDK 版本匹配或超过 `build.gradle.kts` 中的 `jvmToolchain` 设置
...
这创建了一个 .NET 控制台应用的开端，它可以从 user secrets 读取 API 密钥。
...
这创建了一个 MCP 客户端，它将连接到作为命令行参数提供的服务器。然后它列出来自所连接服务器的可用工具。
...
- 客户端使用 `McpClient.CreateAsync()` 初始化，它设置传输类型和运行服务器的命令。
...
如果你正在继续服务器快速开始中的天气教程，你的命令可能看起来像这样：`dotnet run -- path/to/QuickstartWeatherServer`。
...
首先，让我们设置我们的 require 并创建基本的客户端类：
...
你可以在[这里](https://github.com/modelcontextprotocol/quickstart-resources/blob/main/mcp-client-ruby/client.rb)找到完整的 `client.rb` 文件。

## 关键组件解释

### 1. 客户端初始化

- `MCPClient` 类以 nil 引用初始化以便惰性设置
...
- 在迭代工具调用之前一次性构建 assistant 内容
- 通过 `@mcp_client.call_tool` 执行工具调用
- 使用 `chat` 辅助方法包装 Anthropic API 调用
- 用 `result.dig("result", "content")` 提取工具结果内容
- 将工具结果传回给 Claude 以获得最终响应
...
如果你正在继续[服务器快速开始中的天气教程](https://github.com/modelcontextprotocol/quickstart-resources/tree/main/weather-server-ruby)，你的命令可能看起来像这样：`bundle exec ruby client.rb /path/to/weather-server-ruby/weather.rb`
...
- 如果相对路径不起作用，使用绝对路径
- 对于 Windows 用户，确保在路径中使用正斜杠（/）或转义的反斜杠（\\）
- 验证服务器文件有正确的扩展名（Python 为 .py，Node.js 为 .js，或 Ruby 为 .rb）
...
- `Errno::ENOENT`：检查你的服务器路径并确保命令（`ruby`、`python3`、`node`）可用
- `Connection refused`：确保服务器正在运行且路径正确
- `Tool execution failed`：验证工具所需的环境变量已设置
- `Anthropic::Errors::AuthenticationError`：检查你的 `.env` 文件是否有一个有效的 `ANTHROPIC_API_KEY`

</Tab>

<Tab title="Rust">

[你可以在这里找到本教程的完整代码。](https://github.com/modelcontextprotocol/quickstart-resources/tree/main/mcp-client-rust)

## 系统要求

在开始之前，确保你的 Linux 系统满足以下要求：

- 最新稳定版本的 [Rust 和 Cargo](https://www.rust-lang.org/tools/install)
- Anthropic API 密钥（Claude）
- 一个要连接的 Python、Node.js 或可执行的 MCP 服务器
...
[`rmcp`](https://github.com/modelcontextprotocol/rust-sdk) crate 提供 Rust MCP SDK 和子进程传输。本示例使用 [`genai`](https://github.com/jeremychone/rust-genai) crate 向 Claude 发送请求并在模型请求中表示工具。
...
打开 `src/main.rs` 并在你完成以下各节时替换其内容。
...
客户端保存模型 API 客户端、活动的 MCP 会话，以及由所连接服务器公布的工具。

### 客户端初始化

接下来，初始化模型客户端，并在没有 MCP 会话或工具的情况下启动：
...
`genai::Client::default()` 在它发送请求时读取 `ANTHROPIC_API_KEY` 环境变量。

### 服务器连接管理

在 `impl MCPClient` 块内添加这个方法：
...
1. 使用命令行上提供的命令和参数将服务器作为子进程启动
2. 通过 stdio 建立一个 MCP 会话
3. 列出服务器公布的所有工具
4. 将这些工具转换为模型请求中使用的格式

### 转换 MCP 工具

在 `impl MCPClient` 块外添加这个函数：
...
MCP 和模型 API 用相似的信息但不同的 Rust 类型描述工具。`convert_tools` 将每个 MCP 工具的名称、描述和输入 schema 映射到一个 `genai` 工具定义。

### 发送模型请求

在 `impl MCPClient` 内添加这个辅助方法：
...
这将模型请求处理保持在一个地方，并在 API 请求失败时添加有用的上下文。
...
该方法首先将用户的查询和可用工具发送给 Claude。当 Claude 请求工具时，客户端通过 MCP 会话执行每个请求，将结果发送回 Claude，并收集最终的文本响应。
...
该循环接受查询，直到用户输入 `quit` 或关闭标准输入。查询错误会被打印但不会终止客户端。

### 清理

在 `impl MCPClient` 内添加这个方法来停止 MCP 会话和子进程：
...
入口点加载 `.env`，将所有剩余的命令行参数视为服务器命令，连接客户端，启动聊天循环，并确保清理在退出前运行。

### 验证完整的文件

在运行客户端之前，确认 `src/main.rs` 中的各项被放置在正确的作用域：

- `new`、`connect_to_server`、`process_query`、`request_model`、`chat_loop` 和 `cleanup` 是单个 `impl MCPClient` 块内的方法。
- `main` 和 `convert_tools` 是 `impl MCPClient` 块外的函数。

Rust 不要求这些项以特定顺序出现，但方法和自由函数必须被放置在正确的作用域。将你的文件与[完整的 `src/main.rs` 示例](https://github.com/modelcontextprotocol/quickstart-resources/blob/main/mcp-client-rust/src/main.rs)进行比较，然后检查它是否编译：
...
使用 `cargo run --`，后跟你通常用来启动 MCP 服务器的命令：
...
运行不带服务器命令的裸 `cargo run` 会打印用法消息并退出。

<Note>

如果你正在继续[服务器快速开始中的天气教程](https://github.com/modelcontextprotocol/quickstart-resources/tree/main/weather-server-rust)，先构建服务器，然后运行类似这样的命令：`cargo run -- ../weather-server-rust/target/debug/weather`
...
2. **资源管理**
   - 在清理期间始终取消 MCP 会话
   - 确保即使连接或聊天循环操作失败时清理也会运行
   - 避免在一个会话活动时启动第二个服务器

3. **安全**
   - 将 API 密钥安全地存储在 `.env` 中
   - 在允许模型驱动的调用之前审查服务器暴露的工具
   - 只连接到你信任的服务器和可执行命令
...
`cargo run --` 之后的参数必须构成一个完整的命令。被解释执行的服务器脚本需要它们的运行时：
...
如果找不到某个命令，使用它的绝对路径或验证它在你的 `PATH` 中可用。

### 环境文件问题

如果你看到 `Failed to load env file`，确保 `.env` 存在于你运行客户端的目录中。

如果模型请求报告缺少 API 密钥，确认 `.env` 包含：
...
- `Unable to list tools from server`：验证服务器成功启动并通过 stdio 通信
- `Tool call ... failed`：验证服务器工具所需的参数和环境变量
- `Failed to serialize tool result`：检查服务器的响应是否有不受支持或格式错误的内容
