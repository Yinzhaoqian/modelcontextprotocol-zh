...
远程服务器**必须（MUST）**在其指定的 URL 上可公开访问。

## 传输类型

远程服务器应使用 Streamable HTTP 传输。SSE 传输已[弃用](/specification/draft/deprecated)，因此仅为支持既有客户端才发布 `"sse"` 远程条目。远程服务器也可以在不同的 URL 上同时支持两种传输。

通过将 `remotes` 条目的 `type` 属性设为 `"streamable-http"` 或 `"sse"` 来指定传输：
...
远程服务器可以使用 `{curly_braces}`（花括号）表示法定义 URL 模板变量。这使得多租户部署成为可能，其中单个服务器定义可以支持多个带有可配置值的端点：
...
在配置此服务器时，用户提供其 `tenant_id` 值，URL 模板便会被解析为相应的端点（例如 `https://us-cell1.analytics.example.com/mcp`）。
...
可以通过为 `remotes` 条目添加 `headers` 属性来指示 MCP 客户端发送特定的 HTTP 头部：
...
`server.json` 中的 `remotes` 属性可以与 `packages` 属性共存，以便让 MCP 宿主应用选择首选的安装方式。
