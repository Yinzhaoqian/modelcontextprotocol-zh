...
在开始之前，先完成我们的[构建 MCP 服务器](/docs/2025-11-25/develop/build-server)教程会有帮助，这样你就能理解客户端和服务器如何通信。
...
- `MCPClient` 类以会话管理和 API 客户端进行初始化
- 使用 `AsyncExitStack` 进行妥善的资源管理
...
4. **工具名称**
   - 工具名称可以根据[这里](/specification/2025-11-25/server/tools#tool-names)指定的格式进行校验
...
- 具有多种传输类型的多个客户端：`STDIO` 和 `SSE`（Server-Sent Events）
